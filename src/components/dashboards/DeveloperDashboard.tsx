...
const DeveloperDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Filter dev-related projects
      const devProjects = projectsData.filter((p: any) =>
        p.stage === 'Dev'
      );

      // Get ALL dev tasks for dev stage projects
      const devTasks = allTasksData.filter((t: any) =>
        t.type === 'Dev' && devProjects.some((p: any) => p.id === t.projectId)
      );
  ...
  const isTaskInRevision = (task: any, project: any) => {
    ...
    if (task.deliverableId) {
      ...
      if (deliverable) {
        ...
        // Note: For dev tasks, we primarily rely on deliverable status
        // History checking could be added if needed
      }
    }
    return false;
  };
  ...
  return (
    <div className="dashboard premium dev-dashboard">
      {/* Premium Navigation Bar */}
      <nav className="dashboard-nav premium-nav">
        <div className="nav-container">
          <h2 className="logo">Dev Pipeline</h2>
      ...
      <div className="dashboard-content">
        ...
        <div className="dashboard-stats premium-stats dev-stats">
          ...
          <div className={`stat-card premium-stat-card ${filter === 'in_progress' ? 'active' : ''}`}
            ...
            <div className="stat-icon dev-stat-icon" style={{ color: '#3b82f6' }}>
              <FaSpinner />
            </div>
            <div className="stat-content">
          ...
          <div className={`stat-card premium-stat-card ${filter === 'in_review' ? 'active' : ''}`}
            ...
            <div className="stat-icon dev-stat-icon" style={{ color: '#f59e0b' }}>
              <FaEye />
            </div>
            <div className="stat-content">
          ...
          <div className={`stat-card premium-stat-card ${filter === 'completed' ? 'active' : ''}`}
            ...
            <div className="stat-icon dev-stat-icon" style={{ color: '#10b981' }}>
              <FaCheckCircle />
            </div>
            <div className="stat-content">
          ...
          {overdueCount > 0 && (
            <div className="stat-card premium-stat-card overdue-stat">
              <div className="stat-icon dev-stat-icon" style={{ color: '#ef4444' }}>
                <FaExclamationTriangle />
              </div>
              <div className="stat-content">
        ...
        <div className="dev-controls">
          <div className="filter-group">
            ...
            <select
              className="filter-select dev-filter"
              value={filter}
        ...
        {/* Development Queue */}
        <div className="dev-production-queue">
          {projects.length === 0 ? (
            <div className="empty-queue">
              <FaRocket className="empty-icon" />
              <h3>No projects in Development</h3>
              <p>Projects will appear here when moved to Development stage.</p>
            </div>
          ) : (
            Object.values(groupedTasks).map((group: any) => {
              ...
              const remainingTasks = projectTasks.filter((t: any) => !t.isCompleted).length;
              const allTasksCompleted = projectTasks.length > 0 && projectTasks.every((t: any) => t.isCompleted);
                ...
                <div key={project.id} className="project-group-card">
                  <div className="project-group-header">
                    <div className="project-header-left">
                      ...
                      <span className="stage-badge dev-stage">
                        Development
                      </span>
                      {allTasksCompleted && (
                        <span className="launch-ready-badge">
                  ...
                  {remainingTasks > 0 && (
                    <div className="launch-countdown-banner">
                      <FaRocket className="countdown-icon" />
                      <span>{remainingTasks} {remainingTasks === 1 ? 'task' : 'tasks'} remaining before launch</span>
                    </div>
                  )}
                  ...
                  <div className="tasks-grid">
                    {projectTasks.length === 0 ? (
                      <div className="no-tasks-message">
                        <FaCode className="no-tasks-icon" />
                        <p>No development tasks created yet.</p>
                        {canClaimProject && (
                      ...
                      projectTasks.map((task: any) => {
                        ...
                        return (
                          <div
                            key={task.id}
                            className={`task-card dev-task-card ${task.isCompleted ? 'completed' : ''} ${isOverdue ? 'overdue' : ''} ${taskInRevision ? 'revision-task' : ''}`}
                            style={{
                          ...
                          >
                            ...
                            {task.fileUrl && (
                              <div className="task-preview-link">
                                {task.fileUrl.includes('http') ? (
                                  <>
                                  ...
                                  <>
                                    ...
                                    <span className="preview-link-text">{task.fileUrl}</span>
                                  </>
                                )}
                              </div>
                            )}
