...
const SocialMediaDashboard: React.FC = () => {
  ...
  const isTaskInRevision = (task: any, project: any) => {
    ...
    // Note: For Social Media tasks, we primarily rely on deliverable status
    return false;
  };
                    ...
                    {projectTasks.length === 0 ? (
                      ...
                      projectTasks.map((task: any) => {
                        ...
                        return (
                          <div
                            key={task.id}
                            className={`task-card sm-task-card ${task.isCompleted ? 'completed' : ''} ${isOverdue ? 'overdue' : ''} ${taskInRevision ? 'revision-task' : ''}`}
                            style={{
