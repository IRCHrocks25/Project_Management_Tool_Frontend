...
const CopyDashboard: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Always load ALL tasks for Copy dashboard so we can see unassigned work
      const allTasksData = await taskService.getAll();

      // Get ALL Copy tasks first (regardless of project stage or assignment)
      ...
      // Projects that already have Copy tasks
      const projectIdsWithCopyTasks = new Set<string>(
        copyTasks.map((t: any) => t.projectId)
      );
      const projectsWithCopyTasks = allProjectsData.filter((p: any) =>
        projectIdsWithCopyTasks.has(p.id)
      );

      // Projects that are in Copy-related stages even if they don't have Copy tasks yet
      const copyStageProjects = allProjectsData.filter((p: any) =>
        p.stage === 'Copy' || p.stage === 'Copy Revision'
      );

      // Combine both sets of projects and de‑dupe
      const combinedProjectsMap = new Map<string, any>();
      [...projectsWithCopyTasks, ...copyStageProjects].forEach((p: any) => {
      ...
      const visibleCopyTasks = copyTasks.filter((t: any) =>
      ...
      <div className="dashboard" style={{
        ...
        <div style={{
          ...
          <h2 style={{
            ...
            Loading Copy Production Dashboard
          </h2>
          <p style={{
            fontSize: '1rem',
            margin: 0,
  ...
  return (
    <div className="dashboard premium" style={{ display: 'flex', minHeight: '100vh', padding: 0 }}>
      ...
      <div style={{
        ...
        {/* Department List - Only show Copy Writing */}
        <div style={{
          padding: '1rem 0.75rem',
          borderBottom: '1px solid rgba(255, 255, 255, 0.1)'
        }}>
          {departmentMenuItems.filter(item => item.id === 'Copy Writing').map((item) => {
        ...
        <div style={{ marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{
              ...
              <FaCopy style={{ fontSize: '1.5rem', color: 'white' }} />
            </div>
            <div>
          ...
          {tasks.length === 0 ? (
            <div style={{
              ...
              <h3 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#1e293b', marginBottom: '0.5rem' }}>
                No active tasks in Copy Writing
              </h3>
              <p>Tasks will appear here when projects are moved to Copy or Copy Revision stage.</p>
          </div>
          ) : viewMode === 'kanban' ? (
            /* Kanban View */
            <div>
